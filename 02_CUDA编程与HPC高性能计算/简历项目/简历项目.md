...
本目录不是生产代码，而是支撑简历叙述的最小工程证据：CUDA 线程索引、合并访问对比、Reduce 多级归约、GEMM Tiling、Roofline 分析、CPU OpenMP/SIMD。
...
```text
独立实现 CUDA SGEMM/HGEMM Kernel：基于 Shared Memory Tiling（BM=BN=128，BK=8）与线程级寄存器累加（TM=TN=8），
减少 HBM 访问并提升数据复用；使用 Warp Shuffle 完成 Warp 内归约，降低同步开销；进一步通过 WMMA 调用 A100 Tensor Core
实现 FP16 GEMM。在 <M,N,K>、FP16 精度下，相比 PyTorch torch.matmul 达到 <X>x 加速，Nsight Compute 显示
Memory SOL 从 <base>% 提升至 <result>%，数值 max diff <Y>。
```
